...
data1 <- data[,c(1:4, 37:38, 36, 40, 47, 45, 41:42, 43, 46 )]
...
# remove variables we don't need
data2 <- data[,c(1:3, 18, 20, 48:62 )]

# Sorting data
data2 <- data2[with(data2, order(id, year)),]

# create lag
data2$socspend_lag <- unlist(by(data2,data2$id,function(x){c(NA,x[,"socspend"][1:(length(x[,"socspend"])-1)])}))

# set time series
data2$time<- unlist(by(data2,data2$id,function(x) seq(1:nrow(x))))
```
Specification 9: 
```{r}
out9 <- panelAR(socspend ~ socspend_lag + ma_pratio9050s + ma_pratio5010s + ma_pop65+ ma_turnout + ma_fempar + ma_stddisp_gall + ma_pvoc + ma_union + ma_unempl + ma_dreher + gdpgrowth, data=data2, panelVar='id', timeVar='time', autoCorr='psar1', panelCorrMethod='pcse',rho.na.rm=TRUE, bound.rho=TRUE)
summary(out9)
...
```{r}
out11 <- panelAR(socspend ~ mapratio9050s + mapratio5010s +gdpgrowth + as.factor(id), data=data, panelVar='id', timeVar='time', autoCorr='psar1', panelCorrMethod='pcse',rho.na.rm=TRUE, panel.weight='t-1', bound.rho=TRUE)
summary(out11)
```
...
out12 <- panelAR(socspend ~ mapratio9050s + mapratio5010s +gdpgrowth+ as.factor(id), data=data_noout, panelVar='id', timeVar='time', autoCorr='psar1', panelCorrMethod='pcse',rho.na.rm=TRUE, panel.weight='t-1', bound.rho=TRUE)
summary(out12)
```
...
out14<- panelAR(socspend ~ socspend.lag + maratio9010s + maskews + mapop65+ maturnout + mafempar + mastdisp_gall + mapvoc + maunion + maunempl + madreher + gdpgrowth, data=data_noout, panelVar='id', timeVar='time', autoCorr='psar1', panelCorrMethod='pcse',rho.na.rm=TRUE, panel.weight='t-1', bound.rho=TRUE) 
summary(out14)
```
